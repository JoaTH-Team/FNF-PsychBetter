...
class FlxAnimateFunctions
{
	public static function implement(funk:FunkinLua)
	{
		...
		Lua_helper.add_callback(lua, "makeFlxAnimateSprite", function(tag:String, ?x:Float = 0, ?y:Float = 0, ?loadFolder:String = null) {
			tag = tag.replace('.', '');
			var lastSprite = MusicBeatState.getState().variables.get(tag);
			if(lastSprite != null)
			{
				lastSprite.kill();
			...
			MusicBeatState.getState().variables.set(tag, mySprite);
		...
		Lua_helper.add_callback(lua, "loadAnimateAtlas", function(tag:String, folderOrImg:Dynamic, ?spriteJson:Dynamic = null, ?animationJson:Dynamic = null) {
			var spr:FlxAnimate = MusicBeatState.getState().variables.get(tag);
		...
		{
			var obj:Dynamic = MusicBeatState.getState().variables.get(tag);
