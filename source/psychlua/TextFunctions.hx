...
class TextFunctions
{
	public static function implement(funk:FunkinLua)
	{
		...
		function getTextMap():Map<String, flixel.text.FlxText>
		{
			return (game != null) ? game.modchartTexts : StoreLuaVariable.modchartTexts;
		}

		function getObject(name:String):FlxText
		{
			return (game != null) ? LuaUtils.getTextObject(name) : LuaUtils.getTextObject_Other(name);
		}
		...
		Lua_helper.add_callback(lua, "makeLuaText", function(tag:String, text:String, width:Int, x:Float, y:Float) {
			...
			if (game != null) leText.cameras = [game.camHUD];
		...
		Lua_helper.add_callback(lua, "addLuaText", function(tag:String) {
			if (getTextMap().exists(tag)) {
				var shit:FlxText = getTextMap().get(tag);
		...
		Lua_helper.add_callback(lua, "removeLuaText", function(tag:String, destroy:Bool = true) {
			...
			var pee:FlxText = getTextMap().get(tag);
			if(destroy) {
				pee.kill();
			}
